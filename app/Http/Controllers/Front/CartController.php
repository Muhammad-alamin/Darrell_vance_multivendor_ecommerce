    ...
    public function addCart(Request $request)
    {
        ...
        $session_id = Session::get('session_id');
            ...
            if(isset($data['pro_size']))
            {
                ...
                $sizeArr = explode('-', $data['pro_size']);
                $colourArr = explode('-', $data['pro_colour']);

                $attr_pro = Attributes::where(['product_id'=>$data['pro_id'],'attributes_size' => $sizeArr[1]])->first();
//                echo "<pre>";print_r($attr_pro);die;

                $countProducts = DB::table('carts')->where(['pro_id'=>$data['pro_id'],'pro_colour'=>$colourArr[1],'pro_price'=>$data['pro_price'],
                    'pro_size'=>$sizeArr[1],'session_id'=>$session_id])->count();

                $products_quantity = Product::where(['id'=>$data['pro_id']])->first();
                if($countProducts>0)
                {
                    Toastr::error('Product already exists in cart', 'Error', ["positionClass" => "toast-top-right"]);
                    return redirect()->back();
                }
                elseif($data['quantity'] > $attr_pro->attributes_stock)
                {
            ...
            else
                {
                    ...
                    elseif ($data['quantity'] > $products_qty->product_quantity)
                    {
                        Toastr::error('stock not available', 'Error', ["positionClass" => "toast-top-right"]);
                        return redirect()->back();
                    }
...
//            $cartData = Cart::where(['id' => $id,'session_id'=>$session_id])->delete();
    ...
    public function cart(Request $request){
       ...
       foreach ($userCart['carts']as $key=>$products){
           ...
           $userCart['carts'][$key]->image = $productsDetails->product_thumbnail_image;
           $userCart['carts'][$key]->product_quantity = $productsDetails->product_quantity;
       }

//        echo "<pre>";print_r($attrStock);die;
//       dd($userCart);

        return view('front.cartDetails',$userCart);
    }
...
//            dd($availableStock,$data['qty']);
            ...
            //check attribute stock available or not
            if (!empty($availableStock)){
                if($data['qty'] > $availableStock['attributes_stock']){
                    $session_id = Session::get('session_id');
                    ...
                    return response()->json(['status'=>false,'item' => $cartItem]);
                }
            }
            elseif($data['qty'] > $productStock['product_quantity']){
                $session_id = Session::get('session_id');
                $carts = DB::table('carts')->where(['session_id' =>$session_id])->get();
